...
const CheckoutScreen = ({ history }) => {
  ...
  const startPayment = async (ether) => {
    ...
    try {
      ...
      const tx = await signer.sendTransaction({
        to: "0x9E077f23014154A873cF52d68749F04aF5FB5825",
        value: ethers.utils.parseEther(ether.toString()),
      });
      if(tx){
        dispatch(createOrder({amount:Number(cartItems.cartTotal),status:"succeeded"},"Metamask"))
      }
    } catch (err) {
                      ...
                      {paymentMethod && paymentMethod === "Stripe" ? (
                        ...
                        <Button
                          ...
                          onClick={() =>
                            startPayment(
                              cartItems?.couponApplied === true
                                ? Number(cartItems.totalAfterDiscount * 0.000065)
                                : Number(cartItems?.cartTotal * 0.000065)
                            )
                          }
                        >
                          {loading ? (
